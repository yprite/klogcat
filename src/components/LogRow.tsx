...
function rowMiddleText(row: ParsedLogLine) {
  if (row.parseStatus === 'raw') return row.raw
  if (row.sourceType === 'access') return [row.status, row.method, row.url, row.elapsed !== undefined ? `${row.elapsed}ms` : undefined, row.summary, row.trId].filter(Boolean).join(' ')
  if (row.sourceType === 'error') return [row.jsonLogType, row.errorMethod, row.errorPath, row.errorReason ?? row.summary, row.trId ?? row.traceId].filter(Boolean).join(' ')
  return [row.jsonLogType, row.summary, row.trId].filter(Boolean).join(' ')
}

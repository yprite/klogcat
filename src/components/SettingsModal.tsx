...
export function SettingsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { settings, saveSettings, resetSettings, error } = useSettingsStore()
  const [draft, setDraft] = useState<PersistedSettings>(settings ?? defaultSettings)
  useEffect(() => { setDraft(settings ?? defaultSettings) }, [settings, open])
  if (!open) return null
  const errors = validateSettings(draft)
  const setNum = (key: 'initialTailLines' | 'bufferLimit', value: string) => setDraft({ ...draft, [key]: Number(value) })

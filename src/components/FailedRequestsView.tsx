...
export function FailedRequestsView() {
  ...
  return <section data-testid="failed-requests-view" className="min-h-0 flex-1 overflow-auto rounded border border-slate-800 bg-slate-950 p-4">
    <div className="mb-4 border-b border-slate-800 pb-3">
      <p className="text-xs font-semibold uppercase tracking-wide text-yellow-300">{t(language, 'Failed Requests')}</p>
      <h2 className="text-lg font-semibold text-white">{t(language, 'Request-centric investigation layer')}</h2>
    ...
    <div className="grid gap-3 md:grid-cols-4">
      <div className="rounded border border-slate-800 bg-slate-900 p-3">
        <p className="text-xs uppercase text-slate-500">{t(language, 'Source rows available')}</p>
        <p className="mt-1 text-2xl font-semibold text-white">{sourceRowLabel}</p>
      </div>
      <div className="rounded border border-slate-800 bg-slate-900 p-3">
        <p className="text-xs uppercase text-slate-500">{t(language, 'Failed request groups')}</p>
        <p className="mt-1 text-2xl font-semibold text-white">{failedRequests.length}</p>
      </div>
      <div className="rounded border border-slate-800 bg-slate-900 p-3">
        <p className="text-xs uppercase text-slate-500">{t(language, 'Correlation key')}</p>
        <p className="mt-1 font-mono text-sm text-yellow-200">{correlationLabel}</p>
      </div>
      <div className="rounded border border-slate-800 bg-slate-900 p-3">
        <p className="text-xs uppercase text-slate-500">{t(language, 'Raw logs')}</p>
        <p className="mt-1 text-sm text-slate-200">{t(language, 'Preserved as source of truth')}</p>
      </div>
    </div>
    ...
    {failedRequests.length === 0 ? <div className="mt-4 rounded border border-dashed border-slate-700 bg-slate-900/60 p-4 text-sm text-slate-300">
      <p className="font-semibold text-slate-100">{t(language, 'No failed request groups yet')}</p>
      <p className="mt-2">{t(language, '5xx access rows, ERR rows, or exception rows with {key} will appear here.', { key: correlationLabel })}</p>
      ...
      {failedRequests.map((group) => <article key={group.correlationKey} className="rounded border border-slate-800 bg-slate-900/80 p-4 shadow-lg shadow-black/20">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-500">{t(language, 'Correlation')}</p>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_auto]">
          <div>
            <p className="text-xs uppercase text-slate-500">{t(language, 'Request')}</p>
            <p className="mt-1 break-all text-sm font-medium text-slate-100">{requestTitle(group)}</p>
          </div>
          <div>
            <p className="text-xs uppercase text-slate-500">{t(language, 'Failure reason')}</p>
            <p className="mt-1 break-words text-sm text-slate-200">{reasonLabel(group)}</p>
          </div>
          <div>
            <p className="text-xs uppercase text-slate-500">{t(language, 'Evidence')}</p>
            <p className="mt-1 text-sm text-slate-200">{t(language, 'Raw rows')}: {group.rawRows.length}</p>
          </div>
        </div>
      </article>)}
    </div>}

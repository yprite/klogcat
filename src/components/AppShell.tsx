...
export function AppShell({ eventError }: { eventError?: string }) {
  ...
  useEffect(() => { (async () => {
    ...
    if (isTargetPluginEnabled(s?.plugins.targets, 'awsVm') || isTargetPluginEnabled(s?.plugins.targets, 'csvFile')) void useVmStore.getState().loadTargets(s!.plugins.targets)

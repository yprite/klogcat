...
function snapshot(rows: SdkLogRow[], options: Partial<Pick<LogViewerExtensionSnapshot, 'visibleRows' | 'grepQuery'>> = {}): LogViewerExtensionSnapshot {
...
describe('FailedRequestsView', () => {
  ...
  it('keeps full correlated evidence when the query only leaves one row visible', () => {
    const accessFailure: SdkLogRow = { ...row, id: 1, raw: 'access raw', summary: 'GET /api/orders 503', sourceType: 'access', sourceId: 'src-a', request: { status: '503', method: 'GET', url: '/api/orders', elapsed: 87 }, fields: { trId: 'trx-1', status: '503', method: 'GET', url: '/api/orders' } }
    const errorFailure: SdkLogRow = { ...row, id: 2, raw: 'error raw boom', summary: 'IllegalStateException boom', sourceType: 'error', sourceId: 'src-e', error: { method: 'GET', path: '/api/orders', reason: 'db timeout' }, fields: { trId: 'trx-1', errorMethod: 'GET', errorPath: '/api/orders', errorReason: 'db timeout' } }
    const unrelatedFailure: SdkLogRow = { ...row, id: 3, correlationIds: { trId: 'trx-2' }, raw: 'other access raw', summary: 'POST /api/pay 500', sourceType: 'access', sourceId: 'src-other', request: { status: '500', method: 'POST', url: '/api/pay' }, fields: { trId: 'trx-2', status: '500', method: 'POST', url: '/api/pay' } }

    render(<FailedRequestsExtensionView sdk={sdk} snapshot={snapshot([accessFailure, errorFailure, unrelatedFailure], { visibleRows: [errorFailure], grepQuery: 'boom' })} />)

...
function SdkExerciseViewer({ sdk }: LogViewerExtensionProps) {
  useEffect(() => {
    const unsubscribe = sdk.subscribe(() => undefined)
    sdk.getSnapshot()
    sdk.grep.setQuery('from-sdk')
  ...
  return <section data-testid="sdk-exercise-viewer">SDK exercised</section>
}
...
describe('app shell selection scenario', () => {
  ...
  it('passes a working SDK to extensions and cleans up subscriptions on unmount', async () => {
    registerLogViewerExtension({
      id: 'vendor.sdk-exercise',
      ownerId: 'vendor',
      label: 'SDK Exercise',
      description: 'Exercises host SDK calls',
      component: SdkExerciseViewer,
      requestedCapabilities: ['logs.read', 'logs.export', 'grep.write', 'viewer.control'],
      trustLevel: 'trusted-bundled',
    })

    const view = render(<AppShell />)
    fireEvent.click(screen.getByRole('tab', { name: 'SDK Exercise' }))

    expect(await screen.findByTestId('sdk-exercise-viewer')).toBeInTheDocument()
    await waitFor(() => expect(useLogStore.getState().grepQuery).toBe('from-sdk'))

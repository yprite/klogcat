...
import { checkLogPath, startLogStream, stopAllLogStreams, stopLogStream } from '../commands/tauriLogs'
...
describe('tauriLogs browser fallback', () => {
  it('reports a readable runtime error outside Tauri instead of raw invoke internals', async () => {
    ...
    await expect(checkLogPath({ namespace: 'ns', pod: 'pod', container: 'app', sourceType: 'info', filePath: '/tmp/x.log' })).rejects.toThrow('Tauri runtime is unavailable')

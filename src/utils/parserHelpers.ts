...
export function base(json: Record<string, unknown>, raw: string, meta: SourceMeta, receivedAt: number): ParsedLogLineWithoutId {
  return { ...meta, raw, parseStatus: 'parsed', timestamp: str(json.time), epochTime: num(json.epochTime), receivedAt, jsonLogType: str(json.logType), host: str(json.host), service: str(json.service), serviceId: str(json.serviceId), module: str(json.module), submodule: str(json.submodule), trId: str(json.trId), logger: str(json.logger), thread: str(json.thread), body: compactJson(json.body), summary: raw }
}
